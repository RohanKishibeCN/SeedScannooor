...
import { aggregateResults, getTimestamp, writeCsvOutput, writeJsonOutput } from "./output.js";
...
const parseChains = (chainsInput) => {
    if (!chainsInput)
        return undefined;
    const parts = chainsInput.split(",").map((c) => c.trim()).filter(Boolean);
    return parts;
};
const buildNotionPages = (allAddresses, totalUsd, mnemonicIndex, snapshotTime) => {
    const pages = [];
    for (const [chain, addrList] of Object.entries(allAddresses)) {
        const chainName = chain.toLowerCase();
        for (const addrInfo of addrList) {
            const nativeBalance = chainName === "solana" ? (addrInfo.sol ?? 0.0) : (addrInfo.native_balance ?? 0.0);
            pages.push({
...
const scanMnemonic = async (mnemonicStr, chains, depth, maxConcurrent, intervalMs, tatumApiKey, heliusRpcUrl, prices, thresholdUsd, shutdown) => {
    ...
    for (const chain of evmChains) {
        ...
        if (addrs && addrs.length > 0) {
            tasks.push(scanEvmAddresses(tatumApiKey, addrs, chain, maxConcurrent, intervalMs).then((r) => [chain, r]));
        }
    }
    if (scanSolana) {
        const addrs = derived.solana;
        if (addrs && addrs.length > 0) {
            tasks.push(scanSolanaAddresses(heliusRpcUrl, addrs, SOLANA_MINT_ADDRESSES, maxConcurrent, intervalMs).then((r) => ["solana", r]));
        }
    }
    if (tasks.length === 0)
        return null;
    ...
    for (const c of [...evmChains, ...(scanSolana ? ["solana"] : [])]) {
        if (!allAddresses[c])
            allAddresses[c] = [];
    }
    const flat = [];
...
const main = async () => {
    ...
    const cfg = loadConfig({
        ...
        thresholdUsd: Number.isFinite(opts.threshold) ? opts.threshold : undefined
    });
    ...
    console.log(`当前价格: ETH=$${(prices.ethereum ?? 0).toFixed(2)}, SOL=$${(prices.solana ?? 0).toFixed(2)}, USDT=$${(prices.tether ?? 0).toFixed(4)}`);
    ...
    const bar = new cliProgress.SingleBar({ format: "Scanning mnemonics |{bar}| {value}/{total} {percentage}%" }, cliProgress.Presets.shades_classic);
    ...
    for (let idx = 0; idx < mnemonics.length; idx += 1) {
        ...
        bar.update(idx + 1);
        const scanResult = await scanMnemonic(mnemonicStr, cfg.chains, cfg.depth, cfg.maxConcurrent, cfg.scanIntervalMs, cfg.tatumApiKey, cfg.heliusRpcUrl, prices, cfg.thresholdUsd, shutdown);
        if (!scanResult)
            continue;
    ...
    if (notionPages.length > 0) {
        console.log("\n正在写入 Notion...");
        const { success, failed } = await batchWriteToNotion(cfg.notionApiKey, cfg.notionDatabaseId, notionPages, "failed_notion_writes.jsonl", cfg.maxConcurrent);
