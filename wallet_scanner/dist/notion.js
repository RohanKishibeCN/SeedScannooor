...
export const validateDatabase = async (notion, databaseId) => {
    try {
...
const buildProperties = (pageData) => {
    return {
...
const writeFailedEntries = (failedLogPath, entries) => {
    if (entries.length === 0)
        return;
    const lines = entries.map((e) => JSON.stringify(e) + "\n").join("");
    fs.appendFileSync(failedLogPath, lines, "utf-8");
};
export const batchWriteToNotion = async (notionApiKey, databaseId, pages, failedLogPath = "failed_notion_writes.jsonl", maxConcurrent = 10) => {
    if (pages.length === 0) {
    ...
    const tasks = pages.map((pageData, index) => semaphore.withLock(async () => {
        const payload = {
        ...
        try {
            ...
            return { index, ok: false, error: String(e), pageData };
        }
    }));
