...
export const getTimestamp = () => new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
...
export const aggregateResults = (allAddresses, mnemonicIndex, snapshotTime) => {
    const addresses = [];
        ...
        for (const addrResult of results) {
            ...
            if (chainLower === "solana") {
                addresses.push({
                    chain: chainLower,
                    address,
                    sol_balance: addrResult.sol ?? 0.0,
                    usdt: addrResult.usdt ?? 0.0,
                    usdc: addrResult.usdc ?? 0.0,
                    total_usd_value: 0.0
                });
            }
            else {
                addresses.push({
                    chain: chainLower,
                    address,
                    native_balance: addrResult.native_balance ?? 0.0,
                    usdt: addrResult.usdt ?? 0.0,
                    usdc: addrResult.usdc ?? 0.0,
                    total_usd_value: 0.0
                });
...
export const writeJsonOutput = (outputDir, results, thresholdUsd, scanTime) => {
...
export const writeCsvOutput = (outputDir, results, scanTime) => {
    const effectiveScanTime = scanTime ?? getTimestamp();
    fs.mkdirSync(outputDir, { recursive: true });
